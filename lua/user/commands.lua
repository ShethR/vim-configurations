...
-- Register the autocmd with inline callback definition
autocmd("BufWritePre", {
  group = augroup,
  pattern = { "*.cpp", "*.py", "*.lua", "*.yaml", "*.yml" },
  callback = function()
    -- Save cursor position to later restore
    local curpos = vim.api.nvim_win_get_cursor(0)
    ...
    -- Adjust cursor position if it is outside the buffer
    local line_count = vim.api.nvim_buf_line_count(0)
    if curpos[1] > line_count then
      curpos[1] = line_count
    elseif curpos[1] ~= 1 then
      curpos[1] = math.max(curpos[1] - 1, 1)
    end
